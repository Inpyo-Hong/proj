...
import com.project.iip.activity.TodayExerciseActivity
import com.project.iip.dto.Schedule
import com.project.iip.posenet.lib.BodyPart
import com.project.iip.posenet.lib.Person
import com.project.iip.posenet.lib.Posenet
...
//    }
    ...
    if(angle < 90 && squart == true) {
      ...
      if(cnt==4){
        var mediaPlayer = MediaPlayer.create(context, R.raw.four).start()
      }
      if(cnt==5){
        var mediaPlayer = MediaPlayer.create(context, R.raw.five).start()
      }
      if(cnt==6){
        var mediaPlayer = MediaPlayer.create(context, R.raw.six).start()
      }

      if(cnt==7){
        var mediaPlayer = MediaPlayer.create(context, R.raw.seven).start()
      }
      if(cnt==8){
        var mediaPlayer = MediaPlayer.create(context, R.raw.eight).start()
      }
      if(cnt==9){
        var mediaPlayer = MediaPlayer.create(context, R.raw.nine).start()
      }
    ...
    else if(angle > 120 && bent == true){
      var mediaPlayer = MediaPlayer.create(context, R.raw.bent).start()
      bent = false
    }
